  ...
  <div class="stock-k-line-container">
    <div v-for="item in stockNids" :key="item" class="stock-item">
      ...
      <div class="button-group">
        ...
        <button @click="delZhuizonglingshou(item)">暂不追</button>
...
const moveup= async (item) => {
  try {
    ...
    const response = await axios.post('/dev-api/common/stock/moveUpZhuizonglingshou', { tsCode: item }, {
      headers: {
...
const collectZhuizonglingshou= async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/stock/collectZhuizonglingshou', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const delZhuizonglingshou= async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/stock/delZhuizonglingshou', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const fetchStockNids = async () => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.get('/dev-api/common/stock/info/Zhuizonglingshou', {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const bottom = async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/collect/bkBottomZhuizonglingshou', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
