  ...
  <div class="stock-k-line-container">
    <div v-for="item in stockNids" :key="item" class="stock-item">
      ...
      <div class="button-group">
        ...
        <button @click="delZhuizongbaozhuanyinshua(item)">暂不追</button>
...
const moveup= async (item) => {
  try {
    ...
    const response = await axios.post('/dev-api/common/stock/moveUpZhuizongbaozhuanyinshua', { tsCode: item }, {
      headers: {
...
const collectZhuizongbaozhuanyinshua= async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/stock/collectZhuizongbaozhuanyinshua', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const delZhuizongbaozhuanyinshua= async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/stock/delZhuizongbaozhuanyinshua', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const fetchStockNids = async () => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.get('/dev-api/common/stock/info/Zhuizongbaozhuanyinshua', {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const bottom = async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/collect/bkBottomZhuizongbaozhuanyinshua', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
