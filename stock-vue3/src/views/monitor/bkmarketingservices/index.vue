...
const fetchStockNids = async () => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.get('/dev-api/common/stock/info/BKMarketingservices', {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const cancelCollectStock = async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/collect/delBKMarketingservices', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
