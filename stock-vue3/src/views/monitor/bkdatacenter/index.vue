...
const fetchStockNids = async () => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.get('/dev-api/common/stock/info/BKDatacenter', {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
...
const openDetailPageFin = (nid) => {
  const baseUrl = 'https://emweb.eastmoney.com/pc_extendf10/reDatacenterchart.html?code=';
  let detailUrl1 = "";
  if (nid.startsWith('00')) {
...
const cancelCollectStock = async (item) => {
  try {
    const token = getToken(); // 获取JWT Token
    const response = await axios.post('/dev-api/common/collect/delBKDatacenter', { tsCode: item }, {
      headers: {
        'Authorization': `Bearer ${token}` // 添加JWT Token到请求头
      }
    });
